...
class EstadoFinanciero extends StatelessWidget {
  ...
  const EstadoFinanciero(
      {super.key, required this.obtainedEmail, required this.studentName});
      ...
      child: Column(
        ...
        children: [
          ...
          Container(
            ...
            child: Text(
              "Importante: El soporte de pago tiene que tener la siguiente estructura en el nombre: numeroDocumento-año-conceptoPago. Ej: 9397532-2025-pm1",
...
class _SoportePagoButtonsState extends State<_SoportePagoButtons> {
  ...
  String? _checkFileFormat(String value) {
    final regex =
        RegExp(r'^[0-9]+-\d{4}-(mocp|m|ocp|dg|icfes|p|pp|pm[1-9]|pm10)$');
    ...
    try {
      ...
      if (result != null) {
        ...
        final String? checking =
            _checkFileFormat(name.substring(0, name.lastIndexOf('.')));
  ...
  Future<void> _sendSupport() async {
    ...
    if (_selectedFile != null) {
      ...
      try {
        SendSupportModel response =
            await FinancieroRepository.sendSupport(file: _selectedFile!);
    ...
    return Column(
      ...
      children: [
        BotonAzulFinanciero(
            ...
            fontSize: 17.sp,
            prefix: Image.asset('assets/img/pictureIcon.png',
                fit: BoxFit.contain, width: 16, height: 16),
        ...
        (!_isSending)
            ? BotonAzulFinanciero(
                ...
                fontSize: 18.sp,
                paddingRadius:
                    EdgeInsets.symmetric(horizontal: 56, vertical: 22))
            : SizedBox(
                height: 26, width: 26, child: CircularProgressIndicator()),
        SizedBox(height: 20),
        if (_message != null)
          ResultadoFinanciero(
              operationSuccess: _operationSuccess, message: _message),
        SizedBox(height: 20),
      ],
    );
  }
  ...
  Widget build(BuildContext context) {
    return Container(
        ...
        child: Padding(
            ...
            child: FutureBuilder(
                future:
                    FinancieroRepository.getFinancial(role: 1, email: email),
                        ...
                        children: [
                          ...
                          if (data.derechosGradoPago)
                            ItemListaFinanciero(
                                ...
                                itemValue: "\$ ${data.derechosGradoValor}"),
                          if (data.icfesPago)
                            ItemListaFinanciero(
                                itemTitle: "Icfes",
                                itemValue: "\$ ${data.icfesValor}"),
                          ItemListaFinanciero(
                              itemTitle: "Pensiones Pagas",
                              itemValue: "${data.pensionesPagas}"),
                          ItemListaFinanciero(
                              itemTitle: "Pensiones Pendientes",
                              itemValue: "${data.pensionesPendientes}"),
                          ItemListaFinanciero(
                              itemTitle: "Intereses de no pago",
                              itemDescription:
                          ...
                          ItemListaFinanciero(
                              itemTitle: "Total pagado",
                              itemValue: "\$ ${data.totalPagado}",
                              isLastItem: true),
                        ],
                      );
                    } else {
                      return Text(
                        "No hay informacion financiera obtenida.",
                        textAlign: TextAlign.center,
                      );
                    }
                  }
